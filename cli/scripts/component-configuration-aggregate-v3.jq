...
| (any($omissionRows[]?;
    (.sourceFile == "deployment-images.json" or .sourceFile == "statefulset-images.json") and
    (.code == "COMPONENT_CONFIGURATION_STRICT_JSON_REJECTED" or
     .code == "COMPONENT_CONFIGURATION_PROJECTION_FILTER_REJECTED" or
     .code == "COMPONENT_CONFIGURATION_PIPELINE_FAILED" or
     .code == "COMPONENT_CONFIGURATION_OUTPUT_LIMIT_EXCEEDED" or
     (.code | startswith("COMPONENT_CONFIGURATION_KUBERNETES_API_READ_FAILED"))))) as $eligibleReadIncomplete
| ([$configurationRows[] | select(.internalParticipationState == "incomplete") | .componentId]
    + (if $eligibleReadIncomplete and any($configurationRows[]?; .componentId == "pkg:oci/prometheus/prometheus") then ["pkg:oci/prometheus/prometheus"] else [] end)
    | unique) as $incompleteComponents
| ($configurationRows
    | map(select((has("internalParticipationState") | not) and ((.componentId as $id | $incompleteComponents | index($id)) == null)))
    | merge_components) as $mergedRows
...
| ([ $incompleteComponents[]
     | {code: "COMPONENT_CONFIGURATION_DECLARED_ROLE_AMBIGUOUS", reason: "At least one recognized eligible component controller lacked complete source-bound declared context; no subset was retained.", requiredForEvaluation: true, sourceFile: "component-configuration-aggregate", count: 1} ]) as $participationOmissions
| {
    apiVersion: $apiVersion,
    kind: $kind,
    metadata: {
      schemaVersion: (if $adapterVersion == "component-configuration-adapter-v3" then "1.1.0" else "1.0.0" end),
    ...
    omissions: (($omissionRows + $evidenceOmissions + $participationOmissions)
      | map(. + {count: (.count // 1)})

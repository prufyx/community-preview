...
const (
	SyntheticCRIOFrom = "93.0.0"
	SyntheticCRIOTo   = "94.0.0"
)

// GenerateCRIOConstraints returns an empty revision followed by one
// synthetic-test-only CRI-O rule under ephemeral keys. The fictional tuple
// exercises local knowledge selection without adding shipping coverage.
func GenerateCRIOConstraints(now time.Time) (Artifacts, error) {
	...
	active, err := syntheticCRIOConstraintsBundle("2", now)
	if err != nil {
		return Artifacts{}, err
	}
	continued, err := syntheticCRIOConstraintsBundle("3", now)
	if err != nil {
		return Artifacts{}, err
	}
	...
	manifest := Manifest{
		...
		Revisions: []Revision{
			...
			{Revision: "2", Coverage: "synthetic_crio_artifact_name", PackagePath: Revision2Name, PackageDigest: digest(second), BundleDigest: digest(active)},
			{Revision: "3", Coverage: "synthetic_crio_artifact_name_continuation", PackagePath: Revision3Name, PackageDigest: digest(third), BundleDigest: digest(continued)},
		},
	}
...
func syntheticCRIOConstraintsBundle(revision string, now time.Time) ([]byte, error) {
	requirements, err := cncfcheck.ExternalProfileRequirements()
	if err != nil {
		return nil, err
	}
	catalogue, err := cncfcheck.Catalog(false, "cri-o")
	if err != nil || len(catalogue.Projects) != 1 || len(catalogue.Projects[0].Checks) != 1 {
		return nil, errors.New("synthetic CRI-O source is unavailable")
	}
	entry := catalogue.Projects[0].Checks[0]
	entry.Description = "Synthetic-test-only CRI-O artifact named-reference rule for a fictional version tuple."
	var rule map[string]json.RawMessage
	if json.Unmarshal(entry.Rule, &rule) != nil {
		return nil, errors.New("synthetic CRI-O rule is invalid")
	}
	var subject map[string]json.RawMessage
	if json.Unmarshal(rule["subject"], &subject) != nil {
		return nil, errors.New("synthetic CRI-O subject is invalid")
	}
	subject["from"], _ = json.Marshal(SyntheticCRIOFrom)
	subject["to"], _ = json.Marshal(SyntheticCRIOTo)
	rule["subject"], _ = json.Marshal(subject)
	rule["id"], _ = json.Marshal("synthetic.crio-artifact-short-name.93-to-94")
	var evidence map[string]json.RawMessage
	if json.Unmarshal(rule["evidence"], &evidence) != nil {
		return nil, errors.New("synthetic CRI-O evidence is invalid")
	}

...
const (
	SyntheticBuildpacksFrom = "81.0.0"
	SyntheticBuildpacksTo   = "82.0.0"
)

// GenerateBuildpacksConstraints returns an empty revision followed by one
// synthetic-test-only Buildpacks rule under ephemeral keys. The fictional tuple
// exercises local knowledge selection without adding shipping coverage.
func GenerateBuildpacksConstraints(now time.Time) (Artifacts, error) {
	...
	active, err := syntheticBuildpacksConstraintsBundle("2", now)
	if err != nil {
		return Artifacts{}, err
	}
	continued, err := syntheticBuildpacksConstraintsBundle("3", now)
	if err != nil {
		return Artifacts{}, err
	}
	...
	manifest := Manifest{
		...
		Revisions: []Revision{
			...
			{Revision: "2", Coverage: "synthetic_buildpacks_lifecycle_platform_api", PackagePath: Revision2Name, PackageDigest: digest(second), BundleDigest: digest(active)},
			{Revision: "3", Coverage: "synthetic_buildpacks_lifecycle_platform_api_continuation", PackagePath: Revision3Name, PackageDigest: digest(third), BundleDigest: digest(continued)},
		},
	}
...
func syntheticBuildpacksConstraintsBundle(revision string, now time.Time) ([]byte, error) {
	requirements, err := cncfcheck.ExternalProfileRequirements()
	if err != nil {
		return nil, err
	}
	catalogue, err := cncfcheck.Catalog(false, "buildpacks")
	if err != nil || len(catalogue.Projects) != 1 || len(catalogue.Projects[0].Checks) != 1 {
		return nil, errors.New("synthetic Buildpacks source is unavailable")
	}
	entry := catalogue.Projects[0].Checks[0]
	entry.Description = "Synthetic-test-only Buildpacks Lifecycle Platform API plan rule for a fictional version tuple."
	var rule map[string]json.RawMessage
	if json.Unmarshal(entry.Rule, &rule) != nil {
		return nil, errors.New("synthetic Buildpacks rule is invalid")
	}
	var subject map[string]json.RawMessage
	if json.Unmarshal(rule["subject"], &subject) != nil {
		return nil, errors.New("synthetic Buildpacks subject is invalid")
	}
	subject["from"], _ = json.Marshal(SyntheticBuildpacksFrom)
	subject["to"], _ = json.Marshal(SyntheticBuildpacksTo)
	rule["subject"], _ = json.Marshal(subject)
	rule["id"], _ = json.Marshal("synthetic.buildpacks-lifecycle-platform-api.81-to-82")
	var evidence map[string]json.RawMessage
	if json.Unmarshal(rule["evidence"], &evidence) != nil {
		return nil, errors.New("synthetic Buildpacks evidence is invalid")
	}

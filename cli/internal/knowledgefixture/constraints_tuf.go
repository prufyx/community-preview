...
const (
	SyntheticTUFFrom = "101.0.0"
	SyntheticTUFTo   = "102.0.0"
)

// GenerateTUFConstraints returns an empty revision followed by one
// synthetic-test-only TUF rule under ephemeral keys. The fictional tuple
// exercises local knowledge selection without adding shipping coverage.
func GenerateTUFConstraints(now time.Time) (Artifacts, error) {
	...
	active, err := syntheticTUFConstraintsBundle("2", now)
	if err != nil {
		return Artifacts{}, err
	}
	continued, err := syntheticTUFConstraintsBundle("3", now)
	if err != nil {
		return Artifacts{}, err
	}
	...
	manifest := Manifest{
		...
		Revisions: []Revision{
			...
			{Revision: "2", Coverage: "synthetic_tuf_updater_bootstrap_keyword", PackagePath: Revision2Name, PackageDigest: digest(second), BundleDigest: digest(active)},
			{Revision: "3", Coverage: "synthetic_tuf_updater_bootstrap_keyword_continuation", PackagePath: Revision3Name, PackageDigest: digest(third), BundleDigest: digest(continued)},
		},
	}
...
func syntheticTUFConstraintsBundle(revision string, now time.Time) ([]byte, error) {
	requirements, err := cncfcheck.ExternalProfileRequirements()
	if err != nil {
		return nil, err
	}
	catalogue, err := cncfcheck.Catalog(false, "the-update-framework-tuf")
	if err != nil || len(catalogue.Projects) != 1 || len(catalogue.Projects[0].Checks) != 1 {
		return nil, errors.New("synthetic TUF source is unavailable")
	}
	entry := catalogue.Projects[0].Checks[0]
	entry.Description = "Synthetic-test-only TUF Updater bootstrap-keyword rule for a fictional version tuple."
	var rule map[string]json.RawMessage
	if json.Unmarshal(entry.Rule, &rule) != nil {
		return nil, errors.New("synthetic TUF rule is invalid")
	}
	var subject map[string]json.RawMessage
	if json.Unmarshal(rule["subject"], &subject) != nil {
		return nil, errors.New("synthetic TUF subject is invalid")
	}
	subject["from"], _ = json.Marshal(SyntheticTUFFrom)
	subject["to"], _ = json.Marshal(SyntheticTUFTo)
	rule["subject"], _ = json.Marshal(subject)
	rule["id"], _ = json.Marshal("synthetic.tuf-updater-bootstrap-keyword.101-to-102")
	var evidence map[string]json.RawMessage
	if json.Unmarshal(rule["evidence"], &evidence) != nil {
		return nil, errors.New("synthetic TUF evidence is invalid")
	}

...
const (
	SyntheticKubeflowFrom = "101.0.0"
	SyntheticKubeflowTo   = "102.0.0"
)

// GenerateKubeflowConstraints returns an empty revision followed by one
// synthetic-test-only Kubeflow rule under ephemeral keys. The fictional tuple
// exercises local knowledge selection without adding shipping coverage.
func GenerateKubeflowConstraints(now time.Time) (Artifacts, error) {
	...
	active, err := syntheticKubeflowConstraintsBundle("2", now)
	if err != nil {
		return Artifacts{}, err
	}
	continued, err := syntheticKubeflowConstraintsBundle("3", now)
	if err != nil {
		return Artifacts{}, err
	}
	...
	manifest := Manifest{
		...
		Revisions: []Revision{
			...
			{Revision: "2", Coverage: "synthetic_kubeflow_kfp_component_authoring", PackagePath: Revision2Name, PackageDigest: digest(second), BundleDigest: digest(active)},
			{Revision: "3", Coverage: "synthetic_kubeflow_kfp_component_authoring_continuation", PackagePath: Revision3Name, PackageDigest: digest(third), BundleDigest: digest(continued)},
		},
	}
...
func syntheticKubeflowConstraintsBundle(revision string, now time.Time) ([]byte, error) {
	requirements, err := cncfcheck.ExternalProfileRequirements()
	if err != nil {
		return nil, err
	}
	catalogue, err := cncfcheck.Catalog(false, "kubeflow")
	if err != nil || len(catalogue.Projects) != 1 || len(catalogue.Projects[0].Checks) != 1 {
		return nil, errors.New("synthetic Kubeflow source is unavailable")
	}
	entry := catalogue.Projects[0].Checks[0]
	entry.Description = "Synthetic-test-only KFP component-authoring rule for a fictional version tuple."
	var rule map[string]json.RawMessage
	if json.Unmarshal(entry.Rule, &rule) != nil {
		return nil, errors.New("synthetic Kubeflow rule is invalid")
	}
	var subject map[string]json.RawMessage
	if json.Unmarshal(rule["subject"], &subject) != nil {
		return nil, errors.New("synthetic Kubeflow subject is invalid")
	}
	subject["from"], _ = json.Marshal(SyntheticKubeflowFrom)
	subject["to"], _ = json.Marshal(SyntheticKubeflowTo)
	rule["subject"], _ = json.Marshal(subject)
	rule["id"], _ = json.Marshal("synthetic.kubeflow-kfp-component-authoring.101-to-102")
	var evidence map[string]json.RawMessage
	if json.Unmarshal(rule["evidence"], &evidence) != nil {
		return nil, errors.New("synthetic Kubeflow evidence is invalid")
	}

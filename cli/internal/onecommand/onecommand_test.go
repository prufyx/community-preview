...
func TestRunClassifiesThreeWaySplit(t *testing.T) {
	...
	if report.RouteCatalog.TotalNativeRoutes != 170 {
		t.Fatalf("totalNativeRoutes=%d want 170", report.RouteCatalog.TotalNativeRoutes)
	}
	...
	if len(ctxReport.Checks) != 170 {
		t.Fatalf("checks=%d want 170", len(ctxReport.Checks))
	}
	...
	if total != 170 {
		t.Fatalf("summary total=%d want 170 (%+v)", total, s)
	}
...
// core finding of the design: every one of the 170 native check routes
...
// note's "0 of 170" claim needs updating alongside this test.
func TestNoNativeRouteIsFullySatisfiedByVersionAlone(t *testing.T) {
	...
	if len(routes) != 170 {
		t.Fatalf("native routes=%d want 170", len(routes))
	}

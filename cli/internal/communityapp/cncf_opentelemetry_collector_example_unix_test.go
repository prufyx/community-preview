...
func TestSyntheticOpenTelemetryCollectorWalkthrough(t *testing.T) {
	...
	script := filepath.Join(cliRoot, "examples", "cncf", "opentelemetry-collector", "run.sh")
	if info, err := os.Stat(script); err != nil || info.Mode()&0o111 == 0 {
		t.Fatalf("OpenTelemetry Collector walkthrough script unavailable: %v", err)
	}
	...
	if output, err := build.CombinedOutput(); err != nil {
		t.Fatalf("build OpenTelemetry Collector walkthrough binary: %v\n%s", err, output)
	}
	...
	if err := run.Run(); err != nil {
		t.Fatalf("synthetic OpenTelemetry Collector walkthrough: %v\nstdout:\n%s\nstderr:\n%s", err, stdout.String(), stderr.String())
	}
}
